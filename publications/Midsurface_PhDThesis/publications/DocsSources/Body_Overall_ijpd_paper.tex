...
\todo[inline]{Broaden the literature review to provide a more authoritative review of existing research in the field including
commercial tools that already exist in the field (for example CADFix from ITI Transcendata, spatial.com and
others). The literature review should be a critical evaluation of previous work in the field comparing the
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\STATE {\bf Size criterion}: Get Part's Bounding Box's diagonal length ({\bf pBB})
		...
		\WHILE{End of tree has  not reached}
			...
			\IF {feature type is Sweep Based (Extrude, Revolve etc.) or Primitives (Block, Cylinder etc.), Protrusions or Depressions}
			...
			\ENDIF
			\IF {feature type is Application specific like Pin/Bolt/Rivet etc}
...
\todo[inline]{In the condition for sweep based features clarify how the "Union of Bounding boxes" is defined. what happens if two large child features produce a small combined feature? Clarify the meaning of "Keep Master" in later example the master seems to be deleted}
...
\todo[inline]{ The idealization approach needs further explanation. Some of the key challenges that are identified in the literature review are skipped over in the algorithm. For example how are "non-slender" regions handled and how are there solid regions connected to the mid-surface regions? How is the "Extend and Trim of the Midsurface" achieved to accommodate complex junctions types such as L, T and K?. How is the validity of
the Midsurface output checked?}
...
\todo[inline]{Elaborate on what happens to the features that cannot be treated as thin-walled. Clarify what requirements applies to the features to be successfully turned into mid-surface representations.}

\todo[inline]{A summary of the method implementation should be added to the paper to show how the methodology can
be practically applied. Is the current implementation linked to a proprietary CAD package, and what are the
...
\todo[inline]{The method should be tested on a range of standard parts to demonstrate its breadth of application.  The limitations of the approach should be discussed in the context of the wider literature and future work.
What are the limitations of using a history tree approach rather than a b-rep solid as input? Does this
approach have the potential to be applied to any part regardless of geometry, topology or complexity?}
