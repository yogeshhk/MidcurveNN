...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Phase II: Key Idea}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Few Shot: L-Shape Example}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Few Shot: T-Shape (Query)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{LLM Raw Outputs}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{LLM Outputs: Visual Comparison}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Initial Results: Most Models Fail}

Most models fail; only GPT-4 gives a correct answer. Results from various models:
\begin{itemize}
\item \lstinline|llama 7B g4_0 ggml: (8, 17) & (64, 32)|: Wrong.
\item \lstinline|Gemini: [((8.33,5),(8.33, 22.5)), ((8.33, 22.5), (25,22.5)), ((8.33, 22.5), (0,25))]|: Wrong.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Interpretation: Summary}

\begin{itemize}
\item \textbf{GPT-4}: correct answer — sequence-only representation with good prompting is sufficient for simple shapes
\item \textbf{Smaller models (Llama 7B, Gemini, Bard)}: fail on coordinate arithmetic with raw sequence format
\item \textbf{Key insight}: the flat line-sequence representation has limitations for complex shapes; a richer structure (BRep JSON) is needed
\item \textbf{Solution}: switch to BRep JSON + fine-tuning $\Rightarrow$ Qwen2.5-7B achieves MAE\,=\,0.78, PSR\,=\,98\%
\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nemotron-Mini-4B: NVIDIA's Compact Model}
% \begin{columns}[t]
  % \begin{column}[T]{0.55\linewidth}
  \begin{center}
  \includegraphics[width=\linewidth,keepaspectratio]{nemotron_banner}
  \end{center}
  % \end{column}
  % \begin{column}[T]{0.45\linewidth}
  % \begin{itemize}
  % \item \textbf{4B params}, pruned + distilled from Nemotron-4 15B (Minitron technique)
  % \item Group Query Attention (24 query / 8 KV heads) + relu\textsuperscript{2} activation
  % \item 4-bit NF4 quantisation fits in $\sim$2--3\,GB VRAM
  % \item \textbf{Three usage modes}: (1) HF SFTTrainer QLoRA fine-tune, (2) Unsloth-accelerated QLoRA, (3) Few-shot base-model prompting (no training)
  % \item BRep JSON chat format: system $\to$ user (profile) $\to$ assistant (midcurve)
  % \end{itemize}
  % \end{column}
% \end{columns}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nemotron: Few-Shot Prompt }

  \begin{center}
  \includegraphics[width=\linewidth,keepaspectratio]{nemotron_prompt}
  \end{center}
  
  
  {\footnotesize BRep JSON prompt sent to Nemotron-Mini-4B}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nemotron: Few-Shot Prompt  Result}

  \begin{center}
  \includegraphics[width=\linewidth,keepaspectratio]{nemotron_result}
  \end{center}
  
  {\footnotesize Predicted midcurve (red) vs ground truth (green)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{QLoRA Fine-Tuning (Efficient Approach)}
	\begin{itemize}
	\item 4-bit quantised base model (Qwen / Gemma / Mistral 7B)
	\item LoRA adapters on query/value projections: only $\sim$1\% params trained
	\item Instruction-formatted BRep JSON dataset (993 rows)
	\end{itemize}
\begin{lstlisting}[basicstyle=\tiny, breaklines=true, breakatwhitespace=true]
# QLoRA Fine-tuning (pseudocode)
base = AutoModelForCausalLM.from_pretrained(
          "Qwen/Qwen-7B-Chat", load_in_4bit=True)

lora = LoraConfig(r=64, lora_alpha=128,
          target_modules=["q_proj","v_proj"])
model = get_peft_model(base, lora)

def fmt(row):  # instruction format
    return f"Profile: {row['Profile_brep']}\nMidcurve:"

trainer = SFTTrainer(model, train_dataset=dataset,
              formatting_func=fmt, max_seq_length=512)
trainer.train()        # saves adapter weights only
model.save_pretrained("Midcurve-Qwen-Coder-v1")
\end{lstlisting}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Phase II: Results Comparison}

\begin{center}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Approach} & \textbf{Model} & \textbf{PSR\,(\%)} & \textbf{MAE} & \textbf{Topo} \\
\midrule
Few-shot (seq)   & GPT-4              & $\sim$90 & N/A  & $\sim$0.75 \\
Few-shot (seq)   & Gemini / Llama 7B  & $<$30    & N/A  & N/A        \\
Fine-tune        & CodeT5-small       & $\sim$50 & N/A  & N/A        \\
\textbf{QLoRA}   & \textbf{Qwen2.5-7B}& \textbf{98} & \textbf{0.78} & N/A \\
Few-shot (BRep)  & Nemotron-Mini-4B   & 85.7     & N/A  & 0.83\,\textit{(prelim.)} \\
\bottomrule
\end{tabular}
\end{center}

\begin{itemize}
\item[\tiny$\blacktriangleright$] BRep JSON + QLoRA (Qwen2.5-7B): best results — MAE\,=\,0.78, PSR\,=\,98\%
\item[\tiny$\blacktriangleright$] Nemotron-Mini-4B: 2\,GB VRAM, no fine-tuning, 85.7\% valid JSON — \textit{preliminary}
\item[\tiny$\blacktriangleright$] BRep JSON format successfully encodes branched midcurves for all models
\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Phase II: Pros and Cons}
\begin{columns}[t]
  \begin{column}[T]{0.48\linewidth}
  \begin{block}{\textcolor{white}{Advantages}}
    \begin{itemize}
    \item[\tiny$\blacktriangleright$] Exact coordinates: no rasterisation loss
    \item[\tiny$\blacktriangleright$] BRep JSON handles branches natively
    \item[\tiny$\blacktriangleright$] Leverages vast LLM pretraining knowledge
    \item[\tiny$\blacktriangleright$] Easy to add new shapes to dataset
    \end{itemize}
  \end{block}
  \end{column}
  \begin{column}[T]{0.48\linewidth}
  \begin{alertblock}{\textcolor{white}{Limitations}}
    \begin{itemize}
    \item[\tiny$\blacktriangleright$] Coordinates are tokens: digit arithmetic is harder than regression
    \item[\tiny$\blacktriangleright$] Output validity not guaranteed (needs JSON repair pass)
    \item[\tiny$\blacktriangleright$] Inference latency: 2--10\,s on GPU (vs $<$10\,ms for Phase I)
    \item[\tiny$\blacktriangleright$] Nemotron results preliminary; QLoRA fine-tune ongoing
    \end{itemize}
  \end{alertblock}
  \end{column}
\end{columns}
