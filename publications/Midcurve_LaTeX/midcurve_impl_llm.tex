...
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Approach} & \textbf{Model} & \textbf{T shape} & \textbf{Plus shape} \\
\midrule
Few-shot Prompt  & GPT-4      & Correct  & Partial \\
Few-shot Prompt  & Gemini     & Wrong    & Wrong   \\
Few-shot Prompt  & Llama 7B   & Wrong    & Wrong   \\
Fine-tune        & CodeT5-sm  & Partial  & Wrong   \\
Fine-tune QLoRA  & Qwen-7B    & Partial  & Partial \\
\bottomrule
\end{tabular}
\end{center}

\begin{itemize}
\item[\tiny$\blacktriangleright$] GPT-4 is the only model that solved simple shapes via prompting
\item[\tiny$\blacktriangleright$] Fine-tuning shows promise but needs a larger dataset
\item[\tiny$\blacktriangleright$] BRep JSON format successfully encodes branched midcurves
\end{itemize}
\end{frame}
...
\begin{columns}[t]
  ...
  \begin{alertblock}{\textcolor{white}{Limitations}}
    \begin{itemize}
    \item[\tiny$\blacktriangleright$] LLMs lack geometric reasoning (coordinates are ``tokens'')
    \item[\tiny$\blacktriangleright$] Output validity not guaranteed (needs repair)
    \item[\tiny$\blacktriangleright$] GPT-4 only --- smaller models fail
    \item[\tiny$\blacktriangleright$] Fine-tuning requires large GPU and dataset
    \end{itemize}
  \end{alertblock}
  \end{column}
\end{columns}
