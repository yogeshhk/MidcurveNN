...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nemotron: Few-Shot Prompt and Result}
\begin{columns}[t]
...
\end{columns}
\begin{itemize}
\item Valid JSON rate: 85.7\% (6/7 test shapes); Topology accuracy: 0.83; Combined score: 0.56
\item Base model (no fine-tuning) already generates structurally valid BRep output
\end{itemize}
\end{frame}
...
\midrule
Few-shot (seq)   & GPT-4              & $\sim$90 & ---  & $\sim$0.75 \\
Few-shot (seq)   & Gemini / Llama 7B  & $<$30    & ---  & ---        \\
Fine-tune        & CodeT5-small       & $\sim$50 & ---  & ---        \\
\textbf{QLoRA}   & \textbf{Qwen2.5-7B}& \textbf{98} & \textbf{0.78} & --- \\
Few-shot (BRep)  & Nemotron-Mini-4B   & 85.7     & ---  & 0.83\,\textit{(prelim.)} \\
\bottomrule
\end{tabular}
\end{center}
