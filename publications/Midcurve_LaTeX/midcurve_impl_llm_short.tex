...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{QLoRA Fine-tuning Pipeline}
\begin{itemize}
\item Base models: Qwen2.5-Coder, Gemma-2, Mistral (causal LM family)
\item 4-bit NF4 quantisation via BitsAndBytes (fits on consumer GPU)
\item LoRA adapters on all attention projections ($r=16$, $\alpha=32$)
\item SFTTrainer (TRL) with cosine LR schedule + gradient checkpointing
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{SFT Trainer Setup}
\begin{lstlisting}[basicstyle=\tiny]
model = AutoModelForCausalLM.from_pretrained(
    Config.MODEL_ID, quantization_config=bnb_config,
...
trainer = SFTTrainer(
    ...
    tokenizer=tokenizer,
    args=training_args,       # cosine LR, paged_adamw_8bit, ...
    callbacks=[GeometricValidationCallback(...)]
)
trainer.train()
\end{lstlisting}
\end{frame}
...
\begin{center}
\includegraphics[width=0.85\linewidth]{images/shapes_csv}
\end{center}
\begin{itemize}
\item 993 rows total: 80\% train / 10\% val / 10\% test
\item 4 base shapes: I, L, T, Plus --- augmented by rotation, translation, mirroring
\item Columns: \texttt{Profile\_brep} $\to$ \texttt{Midcurve\_brep} (Points / Lines / Segments)
\end{itemize}
...
\begin{itemize}
\item \textbf{JSON validity}: parse success rate (structural correctness)
\item \textbf{Chamfer distance}: average nearest-neighbour distance between point clouds
\item \textbf{Hausdorff distance}: worst-case deviation (max of min distances)
\item \textbf{Topology accuracy}: correct node and edge count vs.\ ground truth
\item \textbf{Combined score}: weighted average of the above metrics
\end{itemize}
\begin{lstlisting}[basicstyle=\tiny]
# From src/text_based/finetuning/metrics_enhanced.py
def compute_all_metrics(pred_brep_str, gt_brep_str):
    """Returns dict with keys: json_valid, chamfer, hausdorff,
       topology_accuracy, combined_score."""
    pred = safe_parse_brep(pred_brep_str)
    gt   = safe_parse_brep(gt_brep_str)
    if pred is None:
        return {"json_valid": 0.0, "chamfer": float('inf'), ...}
    pts_pred = extract_points(pred)
    pts_gt   = extract_points(gt)
    return {"chamfer":   chamfer_distance(pts_pred, pts_gt),
            "hausdorff": hausdorff_distance(pts_pred, pts_gt), ...}
\end{lstlisting}
\end{frame}
