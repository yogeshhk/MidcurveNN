...
\item 2-Manifold vs Non-manifold (\textit{each edge shared by exactly two faces, or not})
...
Example: A surface model of a complex freeform shape
...
\item Graphics, Rapid Prototyping (RP), CAD/CAM, Digital Mock-Up (DMU), CAE
\item Hard to handle
...
\begin{itemize}
\item For a simple polyhedron $ V - E + F = 2$
\begin{itemize}
\item V = Vertices
\item E = Edges
\item F = Faces
...
$4-6+4=2$ \checkmark
\end{itemize}
\note{This formula is used to check whether a solid model is topologically valid. Any valid simple polyhedron must satisfy it.}
\end{frame}
...
\note{This generalization handles solids with through-holes (genus G), inner loops on faces (H), and multiple separate bodies (C). A torus, for example, has G=1.}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{B-Rep vs Mesh}

\begin{itemize}
\item \textbf{B-Rep}: exact boundary; topology (vertices/edges/faces) + geometry (curves/surfaces). Precise, editable.
\item \textbf{Mesh}: approximation by triangles/quads. Lightweight, widely supported (graphics, FEA, 3D printing), but loses exact shape.
\end{itemize}
...
\item NX (Siemens)
\item Catia (Dassault Syst\'{e}mes)
\item Creo (PTC, formerly Pro/Engineer)
\end{itemize}

\item Mid-Range (easier to use)
\begin{itemize}

\item Solid Edge (Siemens)
\item Inventor (Autodesk)
\item SolidWorks (Dassault Syst\'{e}mes)
\end{itemize}
\end{itemize}

They all work basically the same way, \textit{somewhat}!!

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Modern CAD Landscape}
\begin{itemize}
\item Cloud-based: Onshape, Autodesk Fusion 360
\item Open-source: FreeCAD, OpenSCAD (script-based parametric)
\item AI-assisted / generative design is emerging
\item Key file exchange standards: STEP, IGES, STL (for 3D printing)
\end{itemize}
\note{These tools now dominate in education and startups. STEP and IGES are neutral exchange formats; STL is the universal 3D printing format.}
\end{frame}